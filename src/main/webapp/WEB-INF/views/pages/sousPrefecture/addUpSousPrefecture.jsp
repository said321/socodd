								...
								<f:form modelAttribute="sousPrefecture" action="${urlEnregistrer }" role = "form">
                                    ...
                                    <c:if test="${ttt=='nouveau' }">
										<div class="form-group">
		                                    ...
		                                    <select name="pref_id" class = "form-control" id="listPrefectures">
		                                    	<option value = "-1">Select Prefecture</option>
		                                    	<c:forEach items = "${prefectures }" var = "prefecture">
		                                    		<option value = "${prefecture.getId() }" >${prefecture.getNom() }</option>
		                                    	</c:forEach>
		                                    </select>
		                                </div>
	                                </c:if>
	                               ...
	                               <c:if test="${ttt=='modifier' }"> 
		                                <div class="form-group">
		                                    <label>Prefecture</label>
		                                    <select name="pref_id" class = "form-control" id="listPrefectures">
		                                    	<option value = "${sousPrefecture.getPrefecture().getId() }">${sousPrefecture.getPrefecture().getNom() }</option>
		                                    	<c:forEach items = "${sousPrefectures }" var = "sousPrefecture">
		                                    		<c:if test="${sousPrefecture.getPrefecture().getId()!=sousPrefecture.getId() }"> 
